...
func (c *Client) Get(ctx context.Context, resourceGroupName string, VMScaleSetName string, instanceID string, expand compute.InstanceViewTypes) (compute.VirtualMachineScaleSetVM, *retry.Error) {
	...
	if c.RetryAfterReader.After(time.Now()) {
		rerr := retry.GetThrottlingError("VMSSVMGet", fmt.Sprintf("RetryAfter: %s", c.RetryAfterReader.String()))
...
func (c *Client) List(ctx context.Context, resourceGroupName string, virtualMachineScaleSetName string, expand string) ([]compute.VirtualMachineScaleSetVM, *retry.Error) {
	...
	// Report errors if the client is throttled.
	if c.RetryAfterReader.After(time.Now()) {
		rerr := retry.GetThrottlingError("VMSSVMList", fmt.Sprintf("RetryAfter: %s", c.RetryAfterReader.String()))
		return nil, rerr
	}
...
func (c *Client) Update(ctx context.Context, resourceGroupName string, VMScaleSetName string, instanceID string, parameters compute.VirtualMachineScaleSetVM) *retry.Error {
	// Report errors if the client is rate limited.
	...
	if c.RetryAfterWriter.After(time.Now()) {
		rerr := retry.GetThrottlingError("VMSSVMUpdate", fmt.Sprintf("RetryAfter: %s", c.RetryAfterWriter.String()))
		return rerr
	}
