...
func (ss *scaleSet) getPrimarynetworkInterfaceConfiguration(networkConfigurations []VirtualMachineScaleSetNetworkConfiguration, nodeName string) (*VirtualMachineScaleSetNetworkConfiguration, error) {
...
func (ss *scaleSet) getPrimaryIPConfigForScaleSet(config *VirtualMachineScaleSetNetworkConfiguration, nodeName string) (*VirtualMachineScaleSetIPConfiguration, error) {
...
func (ss *scaleSet) EnsureHostInPool(service *v1.Service, nodeName types.NodeName, backendPoolID string, vmSetName string, isInternal bool) error {
	...
	foundPool := false
	newBackendPools := []SubResource{}
	if primaryIPConfiguration.LoadBalancerBackendAddressPools != nil {
		for _, pool := range *primaryIPConfiguration.LoadBalancerBackendAddressPools {
			newBackendPools = append(newBackendPools, SubResource{
				ID: pool.ID,
			})
		}
	}
	...
	newVM := VirtualMachineScaleSetVM{
		Sku:      vm.Sku,
	...
	if ss.CloudProviderBackoff && shouldRetryHTTPRequest(resp, err) {
		...
		if retryErr != nil {
			err = retryErr
			klog.V(2).Infof("EnsureHostInPool update abort backoff vmssVM(%s) with new backendPoolID %s, err: %v", vmName, backendPoolID, err)
		}
	}
...
func (ss *scaleSet) ensureBackendPoolDeletedFromNode(service *v1.Service, nodeName, backendPoolID string) error {
	...
	if ss.CloudProviderBackoff && shouldRetryHTTPRequest(resp, err) {
		...
		if retryErr != nil {
			err = retryErr
			klog.V(2).Infof("ensureBackendPoolDeletedFromNode update abort backoff vmssVM(%s) with backendPoolID %s, err: %v", nodeName, backendPoolID, err)
		}
	}
	if err != nil {
		klog.V(2).Infof("ensureBackendPoolDeletedFromNode failed to update vmssVM(%s) with backendPoolID %s: %v", nodeName, backendPoolID, err)
	} else {
