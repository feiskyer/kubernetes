...
func (c *ManagedDiskController) CreateManagedDisk(options ManagedDiskOptions) (string, error) {
	...
	// Validate and choose zone.
	createAZ := ""
	if options.Zone == nil && options.Zones == nil {
		// TODO: implement SetInformers
		// TODO: cache list of node zones
		// TODO: get candidate zones
		// Query candidate zones if both zone and zones are not specified.
	...
	if options.Zone == nil && options.Zones != nil {
		...
		if adminSetOfZones, err := util.ZonesToSet(*options.Zones); err != nil {
			return "", err
		} else {
	...
	if options.Zone != nil && options.Zones == nil {
		if err := util.ValidateZone(*options.Zone); err != nil {
			return "", err
		}
		createAZ = *options.Zone
	}
